...
var map = global.map = new L.Map('map', {
  contextmenu: true,
  contextmenuItems: [{
    text: 'Bookmark loadshedding group of this position',
...
}).setView([27.6863986, 85.3490229], 13);
...
var Bookmarks = L.Control.extend( /**  @lends Bookmarks.prototype */ {
  ...
  expand: function() {
      ...
      console.log("after expanded: "+this.options.expandedClass)
  ...
  _onBookmarkAdd: function(bookmark) {
    ...
    var self = this;
    var map = this._map;
      console.log("book mark data: "+bookmark.data);
  ...
  _getPopupContent: function(bookmark) {
    if (this.options.getPopupContent) {
      return this.options.getPopupContent.call(this, bookmark);
        ...
        console.log("ty: "+JSON.stringify(bookmark));
...
//        var parameter = JSON.stringify({name:"John Rambo", time:"2pm"});
        var parameter = JSON.stringify(localStorage);
        xmlhttp.open("POST","http://localhost:8080/LoadsheddingMap/index/admin",true);
        xmlhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        xmlhttp.send(parameter);

    }
    passLocalStorageData();

//    testData = localStorage;
//
//    console.log("t data: "+testData)
